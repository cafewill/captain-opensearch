...
const FRAMEWORK = 'node-fastify-with-mdc';
const APP_NAME = process.env.OPENSEARCH_NAME ?? 'simple-jobs-node-fastify-with-mdc';
...
function logJob(jobName) {
  ...
  const msg = `OS+MDC : Batch scheduler job ${fields.jobExecutionId} tracked by node fastify with MDC [${runId}]`;
  console.log(msg);
...
function riskyJob() {
  ...
  const emit = () => {
    ...
    const msg = `OS+MDC : Risky job produced ${level} API latency sample responseTime=${responseTime}ms percentile=${fields.percentileTarget} slowQueryAlarm=${fields.slowQueryAlarm} by node fastify with MDC [${runId}]`;
...
function slowJob() {
  ...
  setTimeout(() => {
    ...
    const msg = `OS+MDC : Slow job method responseTime=${responseTime}ms slowMethodAlarm=${fields.slowMethodAlarm} by node fastify with MDC [${runId}]`;
...
setInterval(() => logJob('system'), SYSTEM_DELAY);
setInterval(() => logJob('manager'), MANAGER_DELAY);
setInterval(() => logJob('operator'), OPERATOR_DELAY);
...
app.listen({ port: PORT, host: '0.0.0.0' }, (err) => {
  ...
  console.log(`simple-jobs-node-fastify-with-mdc running on port ${PORT}`);
});
