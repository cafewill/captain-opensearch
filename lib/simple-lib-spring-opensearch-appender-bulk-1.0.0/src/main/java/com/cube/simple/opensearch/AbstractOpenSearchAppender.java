...
import com.cube.simple.opensearch.config.Authentication;
import com.cube.simple.opensearch.config.OpenSearchProperties;
import com.cube.simple.opensearch.config.HttpRequestHeaders;
import com.cube.simple.opensearch.config.Operation;
import com.cube.simple.opensearch.config.Settings;
import com.cube.simple.opensearch.util.ErrorReporter;

import ch.qos.logback.core.UnsynchronizedAppenderBase;

public abstract class AbstractOpenSearchAppender<T> extends UnsynchronizedAppenderBase<T> {

    protected Settings settings;
    protected OpenSearchProperties opensearchProperties;
    protected AbstractOpenSearchPublisher<T> publisher;
    ...
    public AbstractOpenSearchAppender(Settings settings) {
    ...
    public void start() {
        ...
        try {
            this.publisher = buildOpenSearchPublisher();
        } catch (IOException e) {
    ...
    //VisibleForTesting
    protected abstract AbstractOpenSearchPublisher<T> buildOpenSearchPublisher() throws IOException;
    ...
    public void setProperties(OpenSearchProperties opensearchProperties) {
    ...
    public void setOperation(String operation) {
        settings.setOperation(Operation.of(operation)
                .orElseGet(() -> {
                    addWarn("Invalid value provided for [operation] setting, assuming create. Only 'index' and 'create' are supported.");
    ...
    public void setTrustAllSsl(boolean trustAllSsl) {
        settings.setTrustAllSsl(trustAllSsl);
    }
}
