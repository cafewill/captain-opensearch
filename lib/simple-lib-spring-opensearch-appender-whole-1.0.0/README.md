...
* `com.github.cafewill.opensearch.config.BasicAuthentication` - Supports two configuration methods:
  ...
  * **Legacy**: Credentials in URL (special characters must be URL-encoded, e.g., `@` → `%40`):
    ...
    ```
* `com.github.cafewill.opensearch.config.AWSAuthentication` - Authenticate using the AWS SDK, for use with the [Amazon OpenSearch Service](https://aws.amazon.com/opensearch-service/) (note that you will also need to include `com.amazonaws:aws-java-sdk-core` as a dependency)
 ...
 * The Appender class name is `com.github.cafewill.opensearch.OpenSearchAccessAppender`
