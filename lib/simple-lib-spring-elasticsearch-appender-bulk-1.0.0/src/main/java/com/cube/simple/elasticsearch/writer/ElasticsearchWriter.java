package com.cube.simple.elasticsearch.writer;
...
import com.cube.simple.elasticsearch.config.HttpRequestHeader;
...
public class ElasticsearchWriter implements SafeWriter {
    ...
    public ElasticsearchWriter(ErrorReporter errorReporter, Settings settings, HttpRequestHeaders headers) {
        ...
        for (HttpRequestHeader header : this.headerList) {
            if (header.getName().equalsIgnoreCase(CONTENT_ENCODING) && header.getValue().equals("gzip")) {
            ...
            if (rc == 200) {
                try (InputStream is = urlConnection.getInputStream()) {
                    ...
                    while (is.read(buffer) != -1) {
                        // consume response to enable keep-alive reuse
                    }
                }
            } else {
