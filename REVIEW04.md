...
> 기준은 각 라이브러리의 현재 로컬 소스, `pom.xml`, 예제 `logback-spring.xml`, README 이다. `simple-lib-spring-opensearch-appender-3.0.0`은 `logback-elasticsearch-appender`와 설정 이름을 최대한 맞추되 OpenSearch 내부망 운용을 위해 `trustAllSsl`, `persistentWriterThread`, partial failure 분석을 추가한 구현이다.
