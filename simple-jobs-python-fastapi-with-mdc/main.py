...
FRAMEWORK = 'python-fastapi-with-mdc'
...
appender = OpenSearchJobAppender(
    ...
    app=os.getenv('OPENSEARCH_NAME', 'simple-jobs-python-fastapi-with-mdc'),
...
async def job_loop(fn, delay):
    while True:
        await fn()
        await asyncio.sleep(delay)


async def fixed_rate_job_loop(fn, delay):
    while True:
        asyncio.create_task(fn())
        await asyncio.sleep(delay)


async def log_job(job_name):
    ...
    msg = f'OS+MDC : Batch scheduler job {fields["jobExecutionId"]} tracked by python fastapi with MDC [{run_id}]'
    logger.info(msg)
...
async def do_risky_job():
    ...
    msg = f'OS+MDC : Risky job produced {level} API latency sample responseTime={response_time}ms percentile={fields["percentileTarget"]} slowQueryAlarm={fields["slowQueryAlarm"]} by python fastapi with MDC [{run_id}]'
    logger.warning(msg) if level == 'WARN' else logger.error(msg) if level == 'ERROR' else logger.info(msg)
    appender.log(level, msg, **fields)


async def do_slow_job():
    run_id = str(uuid.uuid4())
    ...
    msg = f'OS+MDC : Slow job method responseTime={response_time}ms slowMethodAlarm={fields["slowMethodAlarm"]} by python fastapi with MDC [{run_id}]'
...
async def lifespan(app: FastAPI):
    asyncio.create_task(job_loop(lambda: log_job('system'), SYSTEM_DELAY))
    asyncio.create_task(job_loop(lambda: log_job('manager'), MANAGER_DELAY))
    asyncio.create_task(job_loop(lambda: log_job('operator'), OPERATOR_DELAY))
    asyncio.create_task(fixed_rate_job_loop(do_risky_job, RISKY_DELAY))
    asyncio.create_task(fixed_rate_job_loop(do_slow_job, SLOW_DELAY))
    yield
...
if __name__ == '__main__':
    ...
    logger.info(f'simple-jobs-python-fastapi-with-mdc running on port {port}')
