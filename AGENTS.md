...
```
AGENTS.md                                          ← 삼순이(claude, chatgpt, gemini) 공통 AI 가이드 및 GitHub 협업 가이드
...
# 공용 라이브러리
lib/simple-lib-spring-opensearch-appender-whole-1.0.0/ ← Spring Boot whole OpenSearch Appender 라이브러리 (com.cube.simple:1.0.0, package com.github.cafewill.opensearch)
lib/simple-lib-spring-opensearch-appender-bulk-1.0.0/  ← Spring Boot bulk OpenSearch Appender 라이브러리 (com.cube.simple:1.0.0, package com.github.cafewill.opensearch)
lib/simple-lib-python-opensearch-appender-1.0.0/   ← Python 공용 OpenSearch Appender 라이브러리 (opensearch-appender:1.0.0)

# 배치잡 앱
simple-jobs-spring-maven-with-java{8,11,17,21,25}/ ← Spring Boot 배치잡 + Maven + Appender 소스 내장
simple-jobs-spring-gradle-with-java{8,11,17,21,25}/ ← Spring Boot 배치잡 + Gradle + Appender 소스 내장
simple-jobs-spring-maven-with-java{8,11,17,21,25}-lib-bulk/ ← Spring Boot 배치잡 + Maven + bulk Appender JAR
simple-jobs-spring-gradle-with-java{8,11,17,21,25}-lib-bulk/ ← Spring Boot 배치잡 + Gradle + bulk Appender JAR
simple-jobs-spring-maven-with-java{8,11,17,21,25}-lib-whole/ ← Spring Boot 배치잡 + Maven + whole Appender JAR
simple-jobs-spring-gradle-with-java{8,11,17,21,25}-lib-whole/ ← Spring Boot 배치잡 + Gradle + whole Appender JAR
...
# REST API 앱
simple-rest-spring-maven-with-java{8,11,17,21,25}/ ← Spring Boot + MyBatis + Maven + Appender 소스 내장 (포트 9201)
simple-rest-spring-gradle-with-java{8,11,17,21,25}/ ← Spring Boot + MyBatis + Gradle + Appender 소스 내장 (포트 9202)
simple-rest-spring-maven-with-java{8,11,17,21,25}-lib-bulk/ ← Spring Boot + MyBatis + Maven + bulk Appender JAR
simple-rest-spring-gradle-with-java{8,11,17,21,25}-lib-bulk/ ← Spring Boot + MyBatis + Gradle + bulk Appender JAR
simple-rest-spring-maven-with-java{8,11,17,21,25}-lib-whole/ ← Spring Boot + MyBatis + Maven + whole Appender JAR
simple-rest-spring-gradle-with-java{8,11,17,21,25}-lib-whole/ ← Spring Boot + MyBatis + Gradle + whole Appender JAR
...
| `application-example.yml` | ✅ 커밋 | 설정 템플릿 (테스트 가능한 기본값 포함) |
| `application.properties` | ❌ gitignore | 실제 사용 설정 (민감 정보 포함) |
| `application.yml` | ❌ gitignore | 실제 사용 설정 (민감 정보 포함) |
...
| `lib/simple-lib-spring-opensearch-appender-bulk-1.0.0/src/main/java/com/github/cafewill/opensearch/OpenSearchAppender.java` | Spring Boot | Job |
| `lib/simple-lib-spring-opensearch-appender-whole-1.0.0/src/main/java/com/github/cafewill/opensearch/OpenSearchAppender.java` | Spring Boot | Job/Web |
| `simple-[jobs|rest]-spring-[maven|gradle]-with-java*/src/main/java/com/github/cafewill/opensearch/OpenSearchAppender.java` | Spring Boot 내장 Appender | Job/Web |
...
| `lib/simple-lib-python-opensearch-appender-1.0.0/opensearch_appender/web_appender_flask.py` | Python / Flask | Web |
| `lib/simple-lib-python-opensearch-appender-1.0.0/opensearch_appender/web_appender_fastapi.py` | Python / FastAPI | Web |
...
| 새 앱 추가 | 신규 어펜더 구현 | .env-example 또는 application-example.properties/yml 생성 | 1-x 앱 목록 갱신 |
| Docker 설정 변경 | - | - | 4절 갱신 |
