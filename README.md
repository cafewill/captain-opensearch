...
```bash
# 1. 라이브러리 디렉터리로 이동
cd lib/simple-lib-spring-opensearch-appender-3.0.0
...
```bash
ls ~/.m2/repository/com/cube/simple-lib-spring-opensearch-appender/3.0.0/
# 아래 두 파일이 있으면 정상
# simple-lib-spring-opensearch-appender-3.0.0.jar
# simple-lib-spring-opensearch-appender-3.0.0.pom
```
...
dependencies {
    implementation 'com.cube:simple-lib-spring-opensearch-appender:3.0.0'
}
```
...
1. `lib/simple-lib-spring-opensearch-appender-3.0.0` 를 로컬 `.m2` 에 설치합니다.
2. 소비 앱 `pom.xml` 또는 `build.gradle` 에 `com.cube:simple-lib-spring-opensearch-appender:3.0.0` 의존성을 둡니다.
3. `application-example.properties` 를 `application.properties` 로 복사합니다.
4. `logback-spring.xml` 에 `OpenSearchAppender` 를 등록하고 `includeMdc=true`, `includeKvp=true`, `operation=create` 같은 값을 설정합니다.
...
|---|---|---|
| 배치잡 | Spring Boot | `lib/simple-lib-spring-opensearch-appender-3.0.0/.../OpenSearchAppender.java` |
...
**① `lib/simple-lib-spring-opensearch-appender-3.0.0` 로컬 설치 (최초 1회)**
