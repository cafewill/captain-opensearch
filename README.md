...
9. [Vision AI 파이프라인 모니터링 — DemoScheduleService MDC 활용](#9-vision-ai-파이프라인-모니터링--demoscheduleservice-mdc-활용)

---

## 0. 시작 전 — 설정 파일 준비

> **보안 주의** : `application.properties`, `application.yml`, `.env` 파일은 `.gitignore` 에 등록되어 있습니다.  
> 각 프로젝트의 예시 파일을 복사한 후 필요시 값을 수정하여 사용하세요.

### Spring Boot 프로젝트

```bash
# 각 Spring Boot 프로젝트 디렉터리에서 해당 형식에 맞게 실행
# Maven 배치잡 앱: application-example.properties 가 있는 경우
cp src/main/resources/application-example.properties src/main/resources/application.properties

# Gradle 배치잡 앱 / REST API 앱: application-example.yml 이 있는 경우
cp src/main/resources/application-example.yml src/main/resources/application.yml
```

예시 파일(`application-example.properties` 또는 `application-example.yml`)에는 로컬 테스트에 필요한 모든 설정이 포함되어 있어  
복사 후 바로 실행 가능합니다.
...
|---|---|---|---|
| `lib/simple-lib-spring-opensearch-appender-whole-1.0.0` | `com.cube.simple` | `simple-lib-spring-opensearch-appender-whole` | `1.0.0` |
| `lib/simple-lib-spring-opensearch-appender-bulk-1.0.0` | `com.cube.simple` | `simple-lib-spring-opensearch-appender-bulk` | `1.0.0` |

Spring Boot 배치잡/REST API 앱이 공통으로 사용하는 OpenSearch Appender 라이브러리.  
Maven Central 에 배포되지 않으므로 `lib/` 아래에서 JAR을 직접 빌드한 뒤 각 Spring Boot 앱이 로컬 파일 의존성으로 참조한다.
...
`bulk` 변형은 Appender Java 패키지를 `com.github.cafewill.opensearch.*` 로 제공하고, `<operation>` 값을 OpenSearch `_bulk` 의 `index` / `create` 액션으로만 제한한다.
...
# 2. Maven Wrapper 로 JAR 생성
#    (mvn 이 전역 설치되어 있으면 mvn package -q -DskipTests 로 대체 가능)
../../simple-jobs-spring-maven-with-java21/mvnw -q -DskipTests package
```

> **Maven Wrapper(`mvnw`)를 쓰는 이유**  
> 이 라이브러리 자체는 `mvnw`를 가지고 있지 않다.  
> 인접한 `simple-jobs-spring-maven-with-java21/mvnw` 를 빌려 실행하면 별도 Maven 설치 없이 동작한다.

#### 빌드 확인

```bash
ls target/simple-lib-spring-opensearch-appender-bulk-1.0.0.jar
```

#### 소비 앱 의존성 선언

`-lib-bulk` / `-lib-whole` 소비 앱은 `lib/` 아래 생성된 JAR을 직접 참조한다.
...
<dependency>
    ...
    <scope>system</scope>
    <systemPath>${project.basedir}/../lib/simple-lib-spring-opensearch-appender-bulk-1.0.0/target/simple-lib-spring-opensearch-appender-bulk-1.0.0.jar</systemPath>
</dependency>
```
...
```gradle
dependencies {
    implementation files('../lib/simple-lib-spring-opensearch-appender-bulk-1.0.0/target/simple-lib-spring-opensearch-appender-bulk-1.0.0.jar')
}
```

#### 재설치가 필요한 경우

라이브러리 소스를 수정한 경우 반드시 JAR을 다시 만든 후 소비 앱을 다시 빌드해야 한다.
...
# 이후 각 소비 앱 재빌드
cd ../../simple-jobs-spring-maven-with-java21-lib-bulk && ./mvnw package -q -DskipTests
cd ../simple-jobs-spring-gradle-with-java21-lib-bulk && ./gradlew build -x test -q
cd ../simple-rest-spring-maven-with-java21-lib-bulk && ./mvnw package -q -DskipTests
cd ../simple-rest-spring-gradle-with-java21-lib-bulk && ./gradlew build -x test -q
```
...
### 1-0-2. Java 버전별 Spring Boot 변형

| 디렉터리 패턴 | Appender 적용 방식 | Appender 패키지 |
|---|---|---|
| `simple-[jobs|rest]-spring-[maven|gradle]-with-java{8,11,17,21,25}` | Appender 소스 내장 | `com.github.cafewill.opensearch` |
| `simple-[jobs|rest]-spring-[maven|gradle]-with-java{8,11,17,21,25}-lib-bulk` | `lib/simple-lib-spring-opensearch-appender-bulk-1.0.0` JAR 사용 | `com.github.cafewill.opensearch` |
| `simple-[jobs|rest]-spring-[maven|gradle]-with-java{8,11,17,21,25}-lib-whole` | `lib/simple-lib-spring-opensearch-appender-whole-1.0.0` JAR 사용 | `com.github.cafewill.opensearch` |

Java 버전별 Spring 앱은 Maven/Gradle, 배치잡/REST API, Appender 소스 내장/lib 사용 변형을 모두 제공한다.  
`-lib-bulk`와 `-lib-whole` 변형은 `lib/` 아래 JAR을 참조하므로 먼저 해당 라이브러리를 `package` 해야 한다.

| 항목 | 소스 내장/whole | bulk |
|---|---|---|
| 기반 라이브러리 | `simple-lib-spring-opensearch-appender-whole-1.0.0` 계열 | `simple-lib-spring-opensearch-appender-bulk-1.0.0` |
...
> 모든 Spring 변형의 `logback-spring.xml` Appender 클래스 경로는 `com.github.cafewill.opensearch.OpenSearchAppender` / `com.github.cafewill.opensearch.config.BasicAuthentication` 를 사용한다.
...
| 디렉터리 | 기술 스택 | 포트 | 빌드 도구 |
|---|---|---|---|
| `simple-jobs-spring-maven-with-java{8,11,17,21,25}` | Spring Boot 배치잡 + Appender 소스 내장 | - | Maven |
| `simple-jobs-spring-gradle-with-java{8,11,17,21,25}` | Spring Boot 배치잡 + Appender 소스 내장 | - | Gradle |
| `simple-jobs-spring-maven-with-java{8,11,17,21,25}-lib-bulk` | Spring Boot 배치잡 + bulk Appender JAR | - | Maven |
| `simple-jobs-spring-gradle-with-java{8,11,17,21,25}-lib-bulk` | Spring Boot 배치잡 + bulk Appender JAR | - | Gradle |
| `simple-jobs-spring-maven-with-java{8,11,17,21,25}-lib-whole` | Spring Boot 배치잡 + whole Appender JAR | - | Maven |
| `simple-jobs-spring-gradle-with-java{8,11,17,21,25}-lib-whole` | Spring Boot 배치잡 + whole Appender JAR | - | Gradle |
...
# Maven + Java 버전별 소스 내장
cd simple-jobs-spring-maven-with-java21
cp src/main/resources/application-example.properties src/main/resources/application.properties
./mvnw spring-boot:run

# Maven + bulk lib JAR
cd simple-jobs-spring-maven-with-java21-lib-bulk
cp src/main/resources/application-example.properties src/main/resources/application.properties
./mvnw spring-boot:run

# Gradle + Java 버전별 소스 내장
cd simple-jobs-spring-gradle-with-java21
cp src/main/resources/application-example.properties src/main/resources/application.properties
./gradlew bootRun
...
1. `lib/simple-lib-spring-opensearch-appender-bulk-1.0.0` 를 `package` 합니다.
2. 소비 앱 `pom.xml` 또는 `build.gradle` 에 `lib/simple-lib-spring-opensearch-appender-bulk-1.0.0/target/*.jar` 참조를 둡니다.
...
`simple-jobs-spring-*-with-java*` 계열은 위 절차가 이미 반영된 샘플입니다.
...
| 디렉터리 | 기술 스택 | 포트 | DB |
|---|---|---|---|
| `simple-rest-spring-maven-with-java{8,11,17,21,25}` | Spring Boot + MyBatis + Appender 소스 내장 | 9201, 9204, 9207, 9210, 9213 | H2 In-Memory |
| `simple-rest-spring-gradle-with-java{8,11,17,21,25}` | Spring Boot + MyBatis + Appender 소스 내장 | 9216, 9219, 9222, 9225, 9228 | H2 In-Memory |
| `simple-rest-spring-maven-with-java{8,11,17,21,25}-lib-bulk` | Spring Boot + MyBatis + bulk Appender JAR | 9203, 9206, 9209, 9212, 9215 | H2 In-Memory |
| `simple-rest-spring-gradle-with-java{8,11,17,21,25}-lib-bulk` | Spring Boot + MyBatis + bulk Appender JAR | 9218, 9221, 9224, 9227, 9230 | H2 In-Memory |
| `simple-rest-spring-maven-with-java{8,11,17,21,25}-lib-whole` | Spring Boot + MyBatis + whole Appender JAR | 9202, 9205, 9208, 9211, 9214 | H2 In-Memory |
| `simple-rest-spring-gradle-with-java{8,11,17,21,25}-lib-whole` | Spring Boot + MyBatis + whole Appender JAR | 9217, 9220, 9223, 9226, 9229 | H2 In-Memory |
...
```bash
# Maven (포트 9201)
cd simple-rest-spring-maven-with-java21
cp src/main/resources/application-example.yml src/main/resources/application.yml
./mvnw spring-boot:run

# Gradle (포트 9202)
cd simple-rest-spring-gradle-with-java21
cp src/main/resources/application-example.yml src/main/resources/application.yml
./gradlew bootRun
```
...
**① `lib/simple-lib-spring-opensearch-appender-bulk-1.0.0` JAR 생성**
...
<appender name="OPENSEARCH" class="com.github.cafewill.opensearch.OpenSearchAppender">
    <url>${OPENSEARCH_URL}</url>
    <index>logs-${OPENSEARCH_NAME}-%date{yyyy.MM.dd}</index>
    <operation>index</operation>
    <authentication class="com.github.cafewill.opensearch.config.BasicAuthentication">
...
```properties
spring.application.name=simple-rest-spring-maven-with-java21
...
```
logs-simple-rest-spring-maven-with-java21-2025.01.15
...
### 3-4. Vision AI MDC 전용 필드 (`DemoScheduleService`)

`simple-jobs-spring-maven-with-java21` 계열 앱의 `DemoScheduleService` Vision AI 파이프라인 잡이  
`MDC.put()` 으로 주입하는 필드입니다. `logback-spring.xml` 의 `<includeMdc>true</includeMdc>` 설정으로  
아래 필드 전체가 OpenSearch 도큐먼트에 자동 포함됩니다.

| 필드 | 타입 | 설명 | 예시 값 |
|---|---|---|---|
| `visionJobId` | keyword | 작업 고유 식별자 (UUID) | `3f2a1b4c-...` |
| `pipeline` | keyword | 파이프라인 종류 | `image-classification`, `object-detection`, `ocr`, `video-analysis` |
| `model` | keyword | 사용 AI 모델명 | `resnet-50`, `yolo-v8`, `tesseract-ocr`, `clip-vit-l14` |
| `inputType` | keyword | 입력 데이터 타입 | `IMAGE`, `VIDEO_FRAME`, `DOCUMENT`, `VIDEO` |
| `stage` | keyword | 처리 단계 | `INFERENCE`, `POSTPROCESS`, `PREPROCESS` |
| `confidence` | float | 추론 신뢰도 (0.000 ~ 1.000) | `0.952` |
| `itemCount` | integer | 처리 항목 수 | `7` |
| `processingTimeMs` | long | 처리 소요 시간 (ms) | `348` |
| `status` | keyword | 처리 결과 | `SUCCESS`, `PARTIAL`, `FAILED` |
...
```bash
# 최근 10건 조회 (최신순)
curl -sk "https://localhost:9200/logs-simple-rest-spring-maven-with-java21-$(date +%Y.%m.%d)/_search" \
  -u admin:Demo3543## \
  -H "Content-Type: application/json" \
  -d '{
    "size": 10,
...
### 5-4-4. Maven Java 21 앱만 최신 10건 조회

```bash
curl -sk -u admin:Demo3543## \
  -H "Content-Type: application/json" \
  "https://localhost:9200/logs-simple-jobs-spring-maven-with-java21-*/_search" \
  -d '{
    "size": 10,
    "sort": [
      { "@timestamp": { "order": "desc" } }
...
### 5-4-5. Gradle Java 21 앱만 최신 10건 조회

```bash
curl -sk -u admin:Demo3543## \
  -H "Content-Type: application/json" \
  "https://localhost:9200/logs-simple-jobs-spring-gradle-with-java21-*/_search" \
  -d '{
    "size": 10,
    "sort": [
      { "@timestamp": { "order": "desc" } }
...
curl -sk "https://localhost:9200/logs-*/_search" \
  ...
  -d '{
    ...
    "query": {
      "bool": {
        "must": [
          { "term":  { "app": "simple-rest-spring-maven-with-java21" } },
...
# 특정 앱 로그
app: "simple-rest-spring-maven-with-java21"
...
## 9. Vision AI 파이프라인 모니터링 — DemoScheduleService MDC 활용

`simple-jobs-spring-maven-with-java21` 계열 앱의 `DemoScheduleService`는 Vision AI 업무 프로세스를  
MDC(Mapped Diagnostic Context)로 구조화하여 OpenSearch에 전송하는 예제입니다.

### 9-1. Vision AI 파이프라인 구성

**기존 잡 (원본 유지)**

| 잡 메서드 | 프로퍼티 키 | 기본 주기 | 설명 |
|---|---|---|---|
| `doSystemJob`   | `job.system.delay`   | 3s  | 시스템 주기 잡 |
| `doManagerJob`  | `job.manager.delay`  | 15s | 매니저 주기 잡 |
| `doOperatorJob` | `job.operator.delay` | 20s | 오퍼레이터 주기 잡 |
| `doRiskyJob`    | `job.risky.delay`    | 60s | 불안정 잡 (WARN/ERROR 발생 가능) |

**Vision AI MDC 신규 잡**

| 잡 메서드 | 프로퍼티 키 | 기본 주기 | 파이프라인 | 모델 | 입력 타입 |
|---|---|---|---|---|---|
| `doImageClassificationJob` | `job.vision.classification.delay` | 5s  | `image-classification` | `resnet-50`     | `IMAGE`       |
| `doObjectDetectionJob`     | `job.vision.detection.delay`      | 18s | `object-detection`     | `yolo-v8`       | `VIDEO_FRAME` |
| `doOcrJob`                 | `job.vision.ocr.delay`            | 25s | `ocr`                  | `tesseract-ocr` | `DOCUMENT`    |
| `doBatchVideoAnalysisJob`  | `job.vision.batch.delay`          | 90s | `video-analysis`       | `clip-vit-l14`  | `VIDEO`       |

### 9-2. MDC 필드 설명

| MDC 필드 | 타입 | 설명 | 예시 값 |
|---|---|---|---|
| `visionJobId` | string | 작업 고유 식별자 | `3f2a1b4c-...` |
| `pipeline` | string | 파이프라인 종류 | `image-classification` |
| `model` | string | 사용 AI 모델 | `yolo-v8` |
| `inputType` | string | 입력 데이터 타입 | `IMAGE`, `VIDEO_FRAME`, `DOCUMENT`, `VIDEO` |
| `stage` | string | 처리 단계 | `INFERENCE`, `POSTPROCESS`, `PREPROCESS` |
| `confidence` | number | 추론 신뢰도 (0~1) | `0.952` |
| `itemCount` | number | 처리 항목 수 | `7` |
| `processingTimeMs` | number | 처리 소요 시간(ms) | `348` |
| `status` | string | 처리 결과 | `SUCCESS`, `PARTIAL`, `FAILED` |

> `logback-spring.xml` 의 `<includeMdc>true</includeMdc>` 설정으로  
> MDC 필드 전체가 OpenSearch 도큐먼트에 자동 포함됩니다.

---

### 9-3. Discover — Vision AI 로그 조회

#### Index Pattern 확인

`logs-*` 인덱스 패턴이 없으면 **6-1** 절차로 먼저 생성하세요.

#### 권장 컬럼 구성

좌측 **Available fields** 패널에서 아래 필드를 추가합니다.
...
# PARTIAL + FAILED 동시 조회
status: "FAILED" or status: "PARTIAL"

# 고신뢰도 추론 결과 (0.9 이상)
confidence >= 0.9

# 처리 지연 감지 — image-classification 500ms 초과
pipeline: "image-classification" and processingTimeMs > 500

# 특정 모델 오류 추적
model: "yolo-v8" and level: "ERROR"

# 배치 영상 분석 이상 감지
pipeline: "video-analysis" and (status: "FAILED" or status: "PARTIAL")

# 특정 작업 전체 흐름 추적
visionJobId: "3f2a1b4c-..."
```

#### Saved Search 저장 (권장)

1. 위 KQL 조건 + 컬럼 설정 완료 후
2. 우측 상단 **Save** → 이름 예시: `VisionAI - video-analysis 이상 감지`
3. 이후 Dashboard 패널로 재사용 가능

---

### 9-4. Visualize — Vision AI 지표 시각화

#### ① 파이프라인별 처리 건수 — Pie Chart
...
| Index | `logs-*` |
| Metrics | Count |
| Buckets > Split slices | Terms / `pipeline.keyword` / Size: 10 |
| Filter | `message: "[VisionAI]" and status: "SUCCESS"` |

#### ② 시간대별 처리 추이 — Area Chart
...
| Y-Axis | Count |
| Split Series | Terms / `pipeline.keyword` |
| Filter | `message: "[VisionAI]"` |

#### ③ 파이프라인별 평균 신뢰도 — Horizontal Bar

| 항목 | 값 |
|---|---|
| 유형 | Horizontal Bar |
| Metrics | Average / `confidence` |
| Buckets | Terms / `pipeline.keyword` |
...
|---|---|
| 유형 | Data Table |
| Metrics | Average + Max + Percentile(95th, 99th) / `processingTimeMs` |
| Buckets | Terms / `pipeline.keyword` |
| Filter | `message: "[VisionAI]" and status: "SUCCESS"` |

#### ⑤ 오류·부분실패 추이 — Line Chart
...
| Y-Axis | Count |
| Split Series | Terms / `status.keyword` |
| Filter | `pipeline: "video-analysis" and (status: "FAILED" or status: "PARTIAL")` |

#### ⑥ 모델별 처리 건수 Top N — Horizontal Bar

| 항목 | 값 |
|---|---|
| 유형 | Horizontal Bar |
| Metrics | Count |
| Buckets | Terms / `model.keyword` / Size: 10 |
| Filter | `message: "[VisionAI]"` |
...
**대시보드명:** `Vision AI Pipeline Monitoring`

| 위치 | 시각화 | 목적 |
|---|---|---|
| 상단 좌 | 파이프라인별 처리 건수 (Pie) | 전체 처리 비중 한눈에 확인 |
| 상단 우 | 시간대별 처리 추이 (Area) | 처리량 변화 및 급증 탐지 |
| 중단 좌 | 파이프라인별 평균 신뢰도 (Horizontal Bar) | 모델 품질 지표 모니터링 |
| 중단 중 | 처리 시간 분포 (Data Table) | 성능 병목 구간 탐지 |
| 중단 우 | 오류·부분실패 추이 (Line) | 장애 조기 감지 |
| 하단 | Saved Search (전체 로그) | 실시간 상세 로그 확인 |

**생성 방법:**

1. 좌측 메뉴 → **Dashboards** → **Create dashboard**
2. **Add** 버튼 → 위에서 저장한 Visualization 선택
3. 드래그로 패널 크기·위치 조정
4. **Save** → 이름: `Vision AI Pipeline Monitoring`
