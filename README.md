...
|---|---|---|---|
| `simple-lib-spring-opensearch-appender-1.2.0` | `com.cube` | `simple-lib-spring-opensearch-appender` | `1.2.0` |
...
```bash
ls ~/.m2/repository/com/cube/simple-lib-spring-opensearch-appender/1.2.0/
# 아래 두 파일이 있으면 정상
# simple-lib-spring-opensearch-appender-1.2.0.jar
# simple-lib-spring-opensearch-appender-1.2.0.pom
```
...
dependencies {
    implementation 'com.cube:simple-lib-spring-opensearch-appender:1.2.0'
}
```
...
> 공통 설계 원칙 : **추가 의존성 없음** — 각 런타임 내장 라이브러리만 사용  
> `OpenSearchJobAppender` (배치잡용) / `OpenSearchWebAppender` (REST API용) 두 종류로 분리
...
| 구분 | OpenSearchJobAppender | OpenSearchWebAppender |
|---|---|---|
| 대상 | 배치잡·스케줄러 | HTTP REST API 서비스 |
| HTTP 추적 | 없음 | `trace_id`, `http_method`, `http_path`, `client_ip`, `http_status`, `duration_ms` |
| 필터/미들웨어 | `MdcJobFilter` (스케줄러 스레드 MDC 자동설정) | `MdcWebFilter` (OncePerRequestFilter) |
| 접근 로그 | 없음 | 요청 완료 시 INFO 로그 자동 발행 |

### 2-3. Spring Boot — 설정 방법

**① `simple-lib-spring-opensearch-appender-1.2.0` 로컬 설치 (최초 1회)**
...
<appender name="OPENSEARCH" class="com.cube.opensearch.OpenSearchJobAppender">
    ...
    <app>${OPENSEARCH_NAME}</app>
    <env>${OPENSEARCH_ENV}</env>
...
```java
// OpenSearchJobAppender.java 내부 static inner class
@Configuration
...
**⑥ MdcWebFilter (REST API) — HTTP 요청 추적**

```java
// OpenSearchWebAppender.java 내부 static inner class
@Component
